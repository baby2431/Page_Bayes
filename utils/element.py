...
def selector(element: Element, is_short = False):
    """
    ...
    p = parent(element)
    if p is not None:
        return selector(p, is_short) + '>' + alias(element, is_short)
    if is_short:
        return element.alias_short
    else:
...
def path_position(element: Element):
    """
    get tag path using recursive function.
    for example result: html/body/div/div/ul/li
    :param element:
...
def alias(element: Element, is_short= False):
    """
